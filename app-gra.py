...
def generate_image(prompt, prompt_2, height, width, num_inference_steps, guidance_scale, num_images_per_prompt):
    ...
    for i, image in enumerate(out_images):
        file_path = os.path.join(save_dir, f'{formatted_now}-{i+1}.png')  # 連番を付けて保存
    ...
    with gr.Row():
        ...
        with gr.Column():
            ...
            height_input = gr.Slider(
                label="画像の高さ (ピクセル)", 
                minimum=256, maximum=2048, step=64, value=768, 
                info="生成される画像の高さを指定します"
            )
            width_input = gr.Slider(
                label="画像の幅 (ピクセル)", 
                minimum=256, maximum=2048, step=64, value=1360, 
            ...
            guidance_scale_input = gr.Slider(
                label="ガイダンススケール", 
                minimum=0.0, maximum=20.0, step=0.1, value=0, 
